...
{
$result = "SELECT * FROM categories where cat_name LIKE '%$search%' OR cat_description LIKE '%$search%' order by id DESC Limit $start, $perpage";
$rows = mysql_query($result);
}
...
if($search == ''){
$qrycount115 = "SELECT count(*) from categories";
...
else
{
  ...
  $qrycount115 = "SELECT count(*) from categories where cat_name LIKE '%$search%' OR cat_description LIKE '%$search%'";
			$resultcount115=mysql_query($qrycount115);
			$row115 = mysql_fetch_array($resultcount115);
			$total115 = $row115[0];
...
<link rel="stylesheet" href="http://maxcdn.bootstrapcdn.com/bootstrap/3.3.5/css/bootstrap.min.css">
  ...
  <script>
    ...
    {
      var x = confirm("Are you sure you want to delete this subject and its relevant questions?");
      if (x)
          return true;
      else
        return false;
...
<div id='welcome'>
<div class="container" >
<h3 style="margin-top:10px; color:#00678F;margin-left:1%;">MANAGE SUBJECTS</h3>
</div>
</div>
		...
		echo "
			...
			<tr>
			   ...
			   <p>{$display['cat_name']}</p>
				...
				<span id='circle'>$total11</span> questions
				<input name='id' id='id' type='hidden' value='{$display['id']}'/>
				<button type='submit' class='btn btn-danger' Onclick='return ConfirmDelete();' title='Deleting this subject will permanately remove it from the system'><i class='fa fa-trash'>delete</i>
</button>
			    </td>
				</form> 
...
{
	echo "<p style='color:#f00;'>No subjects found</p> ";
}
...
echo "<span><a id='page_a_link' href='manage_subjects.php?id={$g}&page=$j'> << </a></span>";
...
echo "<span><a id='page_a_link' href='manage_subjects.php?id={$g}&page=$i'>$i</a></span>";
